...
static const char *const result_type_str[] = {
    [BRIEF] = "Brief", [DETAIL] = "Detail", [DET_LANG] = "Detect Language"};

static const char *const output_mode_str[] = {[PARSE] = "Parse", [RAW] = "Raw"};

/* function array */
static int (*const run_func[])(MoeTr *) = {[PARSE] = parse, [RAW] = raw};

static void (*const parse_func[])(MoeTr *,
                                  cJSON *) = {[BRIEF] = parse_brief,
                                              [DETAIL] = parse_detail,
                                              [DET_LANG] = parse_detect_lang};

#define SET_LANG_PROMPT(SRC, TRG)                                              \
  snprintf(prompt_intrc, sizeof(prompt_intrc), "[ %s:%s ]%s", SRC, TRG,        \
           PROMPT_LABEL)
...
#define BUFFER_REQ_LEN (sizeof(HTTP_REQUEST_DETAIL) + TEXT_ENC_LEN)
  ...
  case BRIEF:
    ret = snprintf(req_buff, BUFFER_REQ_LEN, HTTP_REQUEST_BRIEF,
                   moe->lang_src->code, moe->lang_trg->code, enc_text);
    break;

  case DETAIL:
    ret = snprintf(req_buff, BUFFER_REQ_LEN, HTTP_REQUEST_DETAIL,
                   moe->lang_src->code, moe->lang_trg->code,
                   moe->lang_trg->code, enc_text);
    break;

  case DET_LANG:
    ret = snprintf(req_buff, BUFFER_REQ_LEN, HTTP_REQUEST_DET_LANG, enc_text);
    break;
  ...
  while (b_total < req_len) {
    if ((b_sent = send(moe->sock_d, &req_buff[b_total], req_len - b_total, 0)) <
        0) {
...
static int response_handler(MoeTr *moe) {
  ...
  do {
    ...
    if (b_total == moe->result->size) {
      Memory *new =
          resize_memory(&(moe->result), moe->result->size + (size_t)b_recvd);
...
static int run_intrc(MoeTr *moe) {
  Intrc_cmd prs;
  int ret;
  char *input, *tmp;
...
ch_output:
  ...
  printf("\nMode output has been changed: " REGULAR_YELLOW("%s") "\n\n",
         output_mode_str[out]);
  return OK;
...
ch_result:
  ...
  printf("\nResult type has been changed: " REGULAR_YELLOW("%s") "\n\n",
         result_type_str[res]);
  return OK;
...
static int raw(MoeTr *moe) { return puts(moe->result->val); }
...
static void parse_detail(MoeTr *moe, cJSON *json) {
  ...
  if (cJSON_IsString(src_cor->child)) {
    printf("\n" BOLD_YELLOW("Did you mean: ") "\"%s\" " BOLD_YELLOW("?") "\n\n",
  ...
  cJSON_ArrayForEach(i, synms) {
    ...
    cJSON_ArrayForEach(trg_synn, cJSON_GetArrayItem(i, 2)) {
      ...
      printf("\n" BOLD_WHITE("%d. %s:") "\n   " REGULAR_YELLOW("-> "), iter,
             trg_synn->child->valuestring);

      /* Source alternatives */
      int synn_src_size =
          cJSON_GetArraySize(cJSON_GetArrayItem(trg_synn, 1)) - 1;
...
defs_sect:
  ...
  cJSON_ArrayForEach(i, defs) {
    ...
    cJSON_ArrayForEach(def_subs, cJSON_GetArrayItem(i, 1)) {
      ...
      printf("\n" BOLD_WHITE("%d. %s"), iter, def_subs->child->valuestring);

      def_cre = cJSON_GetArrayItem(def_subs, 3);
      if (cJSON_IsArray(def_cre) && cJSON_IsString(def_cre->child->child)) {

        printf(REGULAR_GREEN(" [ %s ]"), def_cre->child->child->valuestring);
      }
...
static void help_intrc(const MoeTr *moe) {
  printf("------------------------\n" BOLD_WHITE(
         ...
         moe->lang_src->code, moe->lang_trg->code,
         result_type_str[moe->result_type], output_mode_str[moe->output_mode]);
}

static void info_intrc(const MoeTr *moe) {
  printf(BOLD_WHITE("----[ Moetranslate ]----") "\n" BOLD_YELLOW("Interactive "
                                                                 "input mode") "\n\n" BOLD_WHITE(
             "Languages   :") " %s (%s)\n"
                              "              %s (%s)\n" BOLD_WHITE(
                                  "Result type :") " %s\n" BOLD_WHITE("Output "
         ...
         moe->lang_src->value, moe->lang_src->code, moe->lang_trg->value,
         moe->lang_trg->code, result_type_str[moe->result_type],
